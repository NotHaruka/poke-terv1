...
export class BattleAdapter {
    ...
    public createEncounter(playerId: string, wildEntityId: string) {
        ...
        if (playerParty.length === 0 && client.playerData.party) {
            playerParty = client.playerData.party.map((m: any) => monsterInstanceToPokemonInstance(m, { otId: playerId }));
            playerParty.forEach((mon: any, i: number) => {
        ...
        const playerMonsters = client.playerData.party.map((m: any) => ({
