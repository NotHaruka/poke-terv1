...
import {
  ...
  TILE_SIZE, getGlobalTile, isWalkableTileId
...
function handlePlayerInput(gameState: GameState, client: ClientState, packet: PlayerInputPacket): void {
  ...
  // Hard ceiling on input rate (~120/sec) independent of the distance check below, so a
  // client can't flood the server with packets to force excess broadcast work.
  const receivedAt = Date.now();
  if (client.lastInputTime !== undefined && receivedAt - client.lastInputTime < 8) {
    return;
  }
  ...
  // Determine the maximum distance the client could legitimately have covered since
  // its last processed input, based on real server-side wall-clock time (this can't be
  // spoofed by the client, unlike a fixed per-packet tolerance which can be exploited by
  // sending input packets faster than the intended tick rate).
  const now = Date.now();
  const lastTime = client.lastInputTime ?? now;
  // Clamp elapsed time to a sane window: at least one frame (~16.7ms) so the very first
  // packet isn't rejected, and at most 1s so a long gap (e.g. reconnect) doesn't grant a
  // huge one-off allowance.
  const elapsedSec = Math.min(1, Math.max(1 / 60, (now - lastTime) / 1000));
  client.lastInputTime = now;

  const speedPxPerSec = speed * 60; // constants are expressed as px/frame @ 60fps
  const TOLERANCE = 1.5; // allowance for jitter/latency
  ...
  if (packet.position) {
    ...
    // Reject positions that land on a non-walkable tile (walls, trees, water, void, etc.)
    // regardless of distance, so clients can't clip through collision.
...
function handleChunkRequest(gameState: GameState, client: ClientState, packet: ChunkRequestPacket): void {
  ...
  const chunks: ChunkSnapshot[] = packet.chunks.map((c: { cx: number; cy: number }) => ({
    cx: c.cx,
    cy: c.cy,
