import { MOVES_BY_ID, pokemonRegistry } from '@game-core/pokemonData.js';
...
function MonPanel({ mon, name, align }: { mon: MonsterInstance; name: string; align: 'left' | 'right' }) {
  const speciesName = pokemonRegistry.getSpecies(mon.speciesId)?.name || name;
...
export function BattleInterface({
  ...
  message, onSelectMove, onSwitch, onBag, onRun, disabled = false,
}: BattleInterfaceProps) {
  const playerMonName = playerMon.nickname || pokemonRegistry.getSpecies(playerMon.speciesId)?.name || playerName;
  return (
    <div className="w-[640px] rounded-xl border border-neutral-800 bg-gradient-to-b from-neutral-900 to-neutral-950 p-4">
      ...
      <div className="mt-4 rounded-lg border border-neutral-800 bg-black/40 p-3">
        <div className="mb-2 min-h-[1.5rem] text-sm text-neutral-200">
          {message ?? `What will ${playerMonName} do?`}
        </div>

        <div className="grid grid-cols-2 gap-2">
          {playerMon.moves.map((moveId: number) => {
        ...
        <div className="mt-2 flex gap-2">
          <button
            disabled={disabled}
            onClick={onSwitch}
            className="flex-1 rounded border border-neutral-700 bg-neutral-900 px-3 py-2 text-sm text-neutral-300 hover:bg-neutral-800 disabled:opacity-40 cursor-pointer"
          >
            Pokémon
          </button>
          {onBag && (
