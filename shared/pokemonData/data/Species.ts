...
import { LegacyGrowthRate, MonsterSpecies, MonsterStats, MonsterType, LegacyMoveCategory, MoveData } from '../models/PokemonInstance.js';
...
// Backwards compatibility helpers - driven by centralized Database if initialized, otherwise fallback
export const MONSTER_SPECIES: MonsterSpecies[] = new Proxy([] as MonsterSpecies[], {
  get(target, prop, receiver) {
    const db = Database.getInstance();
    let source: MonsterSpecies[] = [];
    if (db.isInitialized()) {
      source = db.getAllPokemon().map(s => ({
        ...
        growthRate: s.growthRate as unknown as LegacyGrowthRate,
        evolutions: [],
      }));
    } else {
      source = Object.values(SPECIES_DATABASE).map(s => ({
        ...
        catchRate: s.catchRate,
        experienceYield: s.baseExp,
        growthRate: LegacyGrowthRate.MediumSlow,
        evolutions: [],
      }));
    }
...
export function getMoveData(id: number): MoveData {
  const db = Database.getInstance();
  if (db.isInitialized()) {
    const move = db.getMove(id);
    if (move) {
      return {
        id: move.id,
        name: move.name,
        type: move.type as unknown as MonsterType,
        category: move.category as unknown as LegacyMoveCategory,
        power: move.power,
  ...
  return {
    ...
    category: LegacyMoveCategory.Physical,
    power: 40,
    accuracy: 100,
    pp: 35,
    priority: 0,
...
export function getDefaultMovesForSpecies(speciesId: number): number[] {
  const db = Database.getInstance();
  if (db.isInitialized()) {
    const learnset = db.getLearnset(speciesId);
    if (learnset && learnset.levelUp && learnset.levelUp.length > 0) {
      const moves = [...learnset.levelUp]
...
export function experienceForLevel(level: number, growthRate: LegacyGrowthRate): number {
  const db = Database.getInstance();
  if (db.isInitialized()) {
    return db.getExpForLevel(level, String(growthRate).toLowerCase());
  }
...
export function getTypeEffectiveness(
  ...
  const db = Database.getInstance();
  if (db.isInitialized()) {
    let mult = db.getTypeEffectiveness(String(attackType), String(defenderTypes[0]));
    if (defenderTypes[1] && defenderTypes[1] !== defenderTypes[0]) {
      mult *= db.getTypeEffectiveness(String(attackType), String(defenderTypes[1]));
    }
    return mult;
  }
  return 1.0;
