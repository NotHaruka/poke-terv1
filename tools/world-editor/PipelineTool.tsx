...
import type { TileDefinition } from '../../packages/shared/types.js';
...
export function PipelineTool() {
  const verifiedCount = TILE_LIBRARY.filter((t: TileDefinition) => t.tags.includes('verified')).length;
  const unverified = TILE_LIBRARY.filter((t: TileDefinition) => !t.tags.includes('verified'));
      ...
      {unverified.length > 0 && (
        <div className="mb-4">
          ...
          <ul className="space-y-0.5 text-xs">
            {unverified.map((t: TileDefinition) => (
